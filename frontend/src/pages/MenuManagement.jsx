...
import AIRecipeSuggestion from "../components/AIRecipeSuggestion.jsx";
...
const MenuManagement = () => {
  ...
  const getIngName = (ing) => {
    ...
    if (typeof ing.ingredient === "object" && ing.ingredient?.name)
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const inventoryItems =
        ingredientsResponse.data.docs || ingredientsResponse.data || [];
      ...
      const finalItems =
        availableItems.length > 0 ? availableItems : inventoryItems;
  ...
  const addIngredientToForm = (ingredient) => {
    const existingIngredient = formData.ingredients.find(
      (ing) => ing.ingredient === ingredient._id
    );
  ...
  if (loading && menuItems.length === 0) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {/* AI Recipe Suggestion or Create Form */}
      {showAIRecipes && (
        <div className="mt-6">
          <AIRecipeSuggestion availableIngredients={availableIngredients} />
        </div>
      )}
                ...
                <div className="border border-gray-300 rounded-lg p-3 min-h-[100px]">
                  {formData.ingredients.map((ing, index) => {
                    const ingredient = availableIngredients.find(
                      (ingredient) => ingredient._id === ing.ingredient
                    );
                    return (
                      <div
                        key={index}
                ...
                <div className="mt-2">
                  {availableIngredients.length === 0 ? (
                    <div className="text-center py-4 text-gray-500">
                      <p>
                        No ingredients available. Please add some inventory
                        items first.
                      </p>
                    </div>
                  ) : (
